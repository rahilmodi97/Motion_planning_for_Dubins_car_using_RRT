...
def environment():
    ...
    obs1 = plt.Rectangle((4,9),1,12, fc='r', ec='k')
    plt.gca().add_patch(obs1)

    obs2 = plt.Rectangle((15,9),1,12, fc='r', ec='k')
    plt.gca().add_patch(obs2)

    obs3 = plt.Rectangle((9.5,-1),1,12, fc='r', ec='k')
    plt.gca().add_patch(obs3)

    obs = plt.Circle((obstacle_1_x, obstacle_1_y), radius=obstacle_rs, fc='r', ec='k')
    plt.gca().add_patch(obs)

    obs = plt.Circle((obstacle_2_x, obstacle_2_y), radius=obstacle_rs, fc='r', ec='k')
    plt.gca().add_patch(obs)
...
def RRT(start_x,start_y,start_theta,goal_x,goal_y,goal_theta):  
    ...
    plt.title("RRT using Dubin's car", fontsize=16, fontweight='bold')                                  #adding title to the plot
...
#Plotting the path that the car has to take from start to goal in red       
    plt.plot([goalNode.pt1_x,goalNode.pt2_x],[goalNode.pt1_y,goalNode.pt2_y],'-r', linewidth=4, alpha=0.5)
    arcf1 = patches.Arc((goalNode.coord1, goalNode.coord2), 2*r_min,2*r_min,theta1=goalNode.theta11, theta2=goalNode.theta12, color='r', linewidth=4, alpha=0.5)
    plt.gca().add_patch(arcf1)
    arcf2 = patches.Arc((goalNode.coord3, goalNode.coord4), 2*r_min,2*r_min,theta1=goalNode.theta13, theta2=goalNode.theta14, color='r', linewidth=4, alpha=0.5)
    plt.gca().add_patch(arcf2)
    for i in path:
        plt.plot([i[0],i[2]],[i[1],i[3]],'-r', linewidth=4, alpha=0.5)
        arcf1 = patches.Arc((i[7], i[8]), 2*r_min,2*r_min,theta1=i[11], theta2=i[12], color='r', linewidth=4, alpha=0.5)
        plt.gca().add_patch(arcf1)
        arcf2 = patches.Arc((i[9], i[10]), 2*r_min,2*r_min,theta1=i[13], theta2=i[14], color='r', linewidth=4, alpha=0.5)
        plt.gca().add_patch(arcf2)
    plt.show()
